...
try:
    import mlflow as _mlflow_real
    _MLFLOW_TRACKING_URI = os.getenv("MLFLOW_TRACKING_URI", "")
    if _MLFLOW_TRACKING_URI:
        _mlflow_real.set_tracking_uri(_MLFLOW_TRACKING_URI)
        _mlflow_real.set_experiment("ai-task-agent")
    import mlflow
except Exception:
    # MLflow unavailable or read-only filesystem — use no-op shim
    import types as _types
    mlflow = _types.SimpleNamespace(
        start_run=lambda *a, **kw: __import__("contextlib").nullcontext(),
